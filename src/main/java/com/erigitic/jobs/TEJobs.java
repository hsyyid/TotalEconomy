...
public class TEJobs
{
	...
	public void checkForLevel(Player player)
	{
		...
		if (playerCurExp >= expToLevel)
		{
			...
			player.sendMessage(Text.of(TextColors.GRAY, "Congratulations, you are now a level ", TextColors.GOLD, playerLevel + 1, " ", jobName, "."));
		}
	}
	...
	public void onPlayerBlockBreak(ChangeBlockEvent.Break event, @Root Player player)
	{
		...
		event.getTransactions().forEach(t -> {
			String blockName = t.getOriginal().getState().getType().getId();
			Location<World> blockLoc = t.getOriginal().getLocation().get();
	...
	public void onPlayerPlaceBlock(ChangeBlockEvent.Place event, @Root Player player)
	{
		...
		event.getTransactions().forEach(t -> {
			String blockName = t.getFinal().getState().getType().getId();
	...
	public void onPlayerFish(FishingEvent.Stop event)
	{
		...
		{
			event.getItemStackTransaction().forEach(itemTransaction -> {
	...
	public void onPlayerClickInventory(ClickInventoryEvent event, @Root Player player)
	{
		if (event.getTargetInventory() instanceof ContainerWorkbench)
		{
			event.getTransactions().forEach(t -> {
				...
				if (slot.slotNumber == 0 && currentCrafters.containsKey(player.getUniqueId()) && currentCrafters.get(player.getUniqueId()).getFirst() == inventory.windowId)
				{
					UUID playerUUID = player.getUniqueId();
					String playerJob = getPlayerJob(player);
					net.minecraft.item.ItemStack result = currentCrafters.get(player.getUniqueId()).getSecond();
					String itemId = result.getItem().getRegistryName().toString();
					...
					if (jobsConfig.getNode(playerJob).getValue() != null)
					{
						if (hasCraftNode && jobsConfig.getNode(playerJob, "craft", itemId).getValue() != null)
						{
							...
							BigDecimal payAmount = new BigDecimal(jobsConfig.getNode(playerJob, "craft", itemId, "pay").getString())
								.multiply(new BigDecimal(result.stackSize)).setScale(2, BigDecimal.ROUND_DOWN);
				...
				else if (slot.slotNumber == 0 && currentCrafters.containsKey(player.getUniqueId()) && currentCrafters.get(player.getUniqueId()).getFirst() != inventory.windowId && inventory.craftResult.getStackInSlot(0) != null)
				{
					currentCrafters.remove(player.getUniqueId());
					currentCrafters.put(player.getUniqueId(), new Tuple<Integer, net.minecraft.item.ItemStack>(inventory.windowId, inventory.craftResult.getStackInSlot(0)));
				}
				else if (slot.slotNumber == 0 && !currentCrafters.containsKey(player.getUniqueId()) && inventory.craftResult.getStackInSlot(0) != null)
				{
					currentCrafters.put(player.getUniqueId(), new Tuple<Integer, net.minecraft.item.ItemStack>(inventory.windowId, inventory.craftResult.getStackInSlot(0)));
				}
			});
		}
		else if (event.getTargetInventory() instanceof ContainerPlayer)
		{
			event.getTransactions().forEach(t -> {
				SlotAdapter slot = (SlotAdapter) t.getSlot();
				ContainerPlayer inventory = (ContainerPlayer) event.getTargetInventory();
