...
@Plugin(id = "totaleconomy", name = "Total Economy", version = "1.4.3")
public class TotalEconomy
{
	...
	public void preInit(GamePreInitializationEvent event)
	{
		...
		defaultCurrency = new TECurrency(Text.of(config.getNode("currency-singular").getValue()), Text.of(config.getNode("currency-plural").getValue()), Text.of(config.getNode("symbol").getValue()), 2, true);
	...
	private void createAndRegisterCommands()
	{
		...
		CommandSpec setBalanceCommand = CommandSpec.builder()
			...
			.arguments(GenericArguments.player(Text.of("player")), GenericArguments.string(Text.of("amount")))
			.build();
	...
	{
		return new BigDecimal(config.getNode("startbalance").getString());
	}
	...
	public boolean isJobPermissions()
	{
